...
contract FlightSuretyData {
    ...
    /**
     * @dev Modifier that requires calling App contract to have been authorized
     */
    modifier requireIsCallerAuthorized() {
        require(
            authorizedContracts[msg.sender] == 1,
            "Unauthorized caller contract"
        );
        _;
    }
        ...
        returns (bool)
    {
        require(
            ...
            "'airlineAddress' must be a valid address."
        );
        require(
     ...
     *
     */
    function buy(string calldata flightCode)
        external
        payable
        requireIsOperational
        returns (
        ...
        )
    {
        ...
        require(
            msg.value > 0,
            "You need to pay something to buy a flight insurance"
        );
    ...
    function checkIfContains(address passenger)
        internal
        view
        returns (bool inList)
    {
     ...
     */
    function creditInsurees(string calldata flightCode)
        external
        requireIsOperational
    {
        for (uint256 c = 0; c < passengerAddresses.length; c++) {
            if (
                ...
                uint256 payedPrice = passengers[passengerAddresses[c]]
                    .boughtFlight[flightCode];
                passengers[passengerAddresses[c]].boughtFlight[flightCode] = 0;
                passengers[passengerAddresses[c]].credit =
                    savedCredit +
                    payedPrice +
                    payedPrice.div(2);
            }
     ...
     *
     */
    function pay(address payable insuredPassenger)
        public
        requireIsOperational
        returns (
            uint256,
        ...
        )
    {
        ...
        require(
            ...
            "The company didn't put any money to be withdrawed by you"
        );
